...
protocol SyncEngineMasterDelegate: class {
    func syncRecordsToCloudKit(recordsToStore: [CKRecord], recordIDsToDelete: [CKRecordID], completion: ((Error?) -> ())?)
}

public final class SyncEngine<T: Object & CKRecordConvertible & CKRecordRecoverable>: SyncEnginable {
    ...
    weak var syncEngineMasterDelegate: SyncEngineMasterDelegate?
...
extension SyncEngine {
    ...
    public func syncObjectsToCloudKit(objectsToStore: [T], objectsToDelete: [T] = []) {
        ...
        syncEngineMasterDelegate?.syncRecordsToCloudKit(recordsToStore: recordsToStore, recordIDsToDelete: recordIDsToDelete, completion: nil)
    }
}
